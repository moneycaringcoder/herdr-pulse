#!/usr/bin/env python3
"""Offline tests for pulse's Herdr API contract check.
...
# reads them directly.
FILLER_OBJECTS = ("TabInfo", "PaneInfo")
...
def contract_schema(protocol: int = MIN_PROTOCOL) -> dict[str, Any]:
    ...
    return {
        ...
        "schema_version": 1,
        "schemas": {
            "request": {"oneOf": [request_variant(method) for method in sorted(REQUESTS)]},
...
class LocalReferenceTests(unittest.TestCase):
    def test_parameters_behind_a_local_reference_are_followed(self) -> None:
        ...
        del schema["schemas"]["request"]["$defs"]["Params"]["properties"]["workspace_id"]
        with self.assertRaisesRegex(ContractError, "no longer accepts `workspace_id`"):
            validate(schema)
